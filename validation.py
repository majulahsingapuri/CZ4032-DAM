...
from RG import rule_generator, is_satisfy
from M2 import M2_classifier
import time
import random
...
def cross_validate_m2_without_prune(data_path, scheme_path, minsup=0.01, minconf=0.5):
    data, attributes, value_type = readfile(data_path, scheme_path)
    random.shuffle(data)
    ...
    for k in range(len(split_point)-1):
        ...
        start_time = time.time()
        classifier_m2 = M2_classifier(cars, training_dataset)
        end_time = time.time()
        ...
        total_classifier_rule_num += len(classifier_m2.rulelist)
        ...
        print("No. of rules in classifier of CBA-CB M2 without pruning: %d" % len(classifier_m2.rulelist))
...
def cross_validate_m2_with_prune(data_path, scheme_path, minsup=0.01, minconf=0.5):
    ...
    for k in range(len(split_point)-1):
        ...
        start_time = time.time()
        cars = rule_generator(training_dataset, minsup, minconf)
        cars.pruneRules(training_dataset)
        ...
        print("No. of rules in classifier of CBA-CB M2 with pruning: %d" % len(classifier_m2.rulelist))
